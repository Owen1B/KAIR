...
def main():
    ...
    # =========================================
    config = {
        'input_dir': 'testsets/spectH_clinical',  # 输入数据目录（临床数据）
        'output_base_dir': 'SPECTdatasets',  # 输出基础目录
    }

    # 创建所有输出目录
    output_dirs = {
        'binomial_4x': os.path.join(config['output_base_dir'], 'spectL_clinical_binomial_4x'),
        'binomial_8x': os.path.join(config['output_base_dir'], 'spectL_clinical_binomial_8x'),
    }
    ...
    print('生成的数据集：')
    print('1. 二项重采样图：')
    print('   - spectL_XCAT_clinical_binomial_4x: 临床数据的0.25倍采样')
    print('   - spectL_XCAT_clinical_binomial_8x: 临床数据的0.125倍采样')
